...
def load_config(map_size, mm_mode, pheromone_mode, pheromone_decay):
    gw = magent.gridworld
    cfg = gw.Config()

    cfg.set({"map_width": map_size, "map_height": map_size})
    cfg.set({"minimap_mode": mm_mode})
    cfg.set({"pheromone_mode": pheromone_mode > 0}) # Can agents see pheromones? (always of its own group)
    cfg.set({"pheromone_decay": pheromone_decay}) # 0.05
    cfg.set({"embedding_size": 10})

    nopher = cfg.register_agent_type(
        "nopher",
        {'width': 1, 'length': 1, 'hp': 10, 'speed': 2,
         ...
         'can_lay_pheromone': int(pheromone_mode == 2)
         })

    pher = cfg.register_agent_type(
        "pher",
        {'width': 1, 'length': 1, 'hp': 10, 'speed': 2,
         'view_range': gw.CircleRange(6), 'attack_range': gw.CircleRange(1.5),
         'damage': 2, 'step_recover': 0.1,
         'step_reward': -0.005,  'kill_reward': 5, 'dead_penalty': -0.1, 'attack_penalty': -0.1,
         'can_lay_pheromone': int(pheromone_mode >= 1)
         })

    g0 = cfg.add_group(nopher)
    g1 = cfg.add_group(pher)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", type=str, default="battle2")
    ...
    parser.add_argument("--pheromone_mode", type=int, default=0)
